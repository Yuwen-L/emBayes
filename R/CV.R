...
cv <- function(y,clin,X,quant,t0,t1,k,func,error,maxiter){
  l1 <- length(t0)
  l2 <- length(t1)
  n <- nrow(X)
  C <- cbind(1,clin)
  ...
  for(i in 1:l1){
      for(j in 1:l2){
        ...
        for(f in 1:k){
          k.sub <- which(folds==f)
          if(func=="BLSS"){
            EM <- BLSSEM(y[-k.sub], clin[-k.sub,], X[-k.sub,],t0[i],t1[j],error,maxiter)
          }
          else{
            EM <- BQLSSEM(y[-k.sub], clin[-k.sub,], X[-k.sub,],quant,t0[i],t1[j],error,maxiter)
          }
          beta_k <- EM$beta
          alpha_k <- EM$alpha
